...
import { Container, Button, Card, Dropdown, Col, Row } from "react-bootstrap";
...
function Profile() {
  ...
  useEffect(() => {
    const fetchProfileAndFavorites = async () => {
      ...
      try {
        ...
        const favouritesResponse = await fetch(`http://localhost:5000/api/favourites/${data.id}`, {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
        ...
        // Fetch user reviews
        const reviewsResponse = await fetch(`http://localhost:5000/api/user-reviews/${data.id}`, {
          method: "GET",
          headers: {
            "Content-Type": "application/json",
            Authorization: `Bearer ${token}`,
  ...
  const handleRemoveFavorite = async (animeId) => {
    ...
    try {
      const response = await fetch(`http://localhost:5000/api/remove-favourites/${user.id}/${animeId}`, {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          Authorization: `Bearer ${token}`,
      ...
      <Container style={{ maxWidth: "1200px", padding: "20px" }}>
        ...
        <Box sx={{ marginTop: "20px", backgroundColor: "#121317", color: "whitesmoke", padding: "20px", borderRadius: "8px" }}>
          ...
          sx={{
            ...
            <Tab label={`Review History (${user.reviews.length})`} />
          </Tabs>
               ...
               {user?.reviews?.length > 0 ? (
                 user.reviews.map((review, index) => (
                   <Card key={index} style={{ marginBottom: "10px", backgroundColor: "#1a1a1a" }}>
                     <Card.Body>
                        <Row>
                          <h6 className="purple">{user.username}  <span className="grey"><FaPlay style={{fontSize:'10px', marginRight:'5px', marginLeft:'5px'}} />{review.title}</span> </h6>
