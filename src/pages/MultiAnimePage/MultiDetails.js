...
function MultiDetails() {
  ...
  useEffect(() => {
    ...
    fetch(`${API_URL}/anime/top100`)  // Update the URL to match your backend route
      ...
      .then(data => {
        const topAnime = data.top_100_anime || [];
        setAnimeData(topAnime);  // Ensure we always have an array, even if data is empty
        setLoading(false);  // Set loading to false when data is loaded
        setFilteredData(topAnime); // Set filtered data initially
      })
      .catch(err => {
